...
pub fn render_game_screen(app: &mut LexivoApp, ui: &mut egui::Ui) {
    ...
    render_header_score(app, ui, metrics);
    ui.add_space(8.0);
    ...
    if app.timer_enabled {
        ...
        ui.label(format!("Time left: {minutes}:{seconds:02}"));
    } else {
        ui.label("Timer: Off");
    }
    ...
    if app.game_over {
        render_game_over(app, ui, palette, metrics);
        return;
    }

    ui.add_space(25.0);
    render_scrambled_tiles(app, ui, palette, metrics);
    ui.add_space(8.0);
    render_hint(app, ui, palette, metrics);
    ui.add_space(4.0);
    render_answer_slots(app, ui, palette, metrics);
    ui.add_space(40.0);
    render_action_buttons(app, ui, metrics);
}

fn render_header_score(app: &LexivoApp, ui: &mut egui::Ui, metrics: crate::theme::ThemeMetrics) {
    ui.heading(
        egui::RichText::new(AppConfig::APP_NAME.to_uppercase())
            .size(metrics.game_heading_size)
            .strong(),
        ...
        for (idx, c) in scrambled.chars().enumerate() {
            if draw_tile(
                ...
                TileOptions {
                    ...
                    index: idx,
                    time,
                    fill_colour: palette.tile_fill,
                    text_colour: palette.tile_text,
                    metrics,
...
fn render_hint(
    ...
    let hint = app.puzzles[app.current_index].hint.clone();
    ui.group(|ui| {
        ui.set_min_width(metrics.hint_group_min_width);
        ui.label(
...
fn render_answer_slots(
    app: &mut LexivoApp,
    ui: &mut egui::Ui,
    palette: &crate::theme::Palette,
    metrics: crate::theme::ThemeMetrics,
) {
    let answer_len = app.puzzles[app.current_index].answer.chars().count();
    ...
    ui.horizontal_wrapped(|ui| {
        ...
        for idx in 0..answer_len {
            if let Some(ch) = slots.get(idx).copied().flatten() {
                if render_filled_slot(ui, idx, ch, app, palette, metrics) {
                    remove_index = Some(idx);
                }
            } else {
                render_empty_slot(ui, idx, app, slot_size);
            }
            if idx < answer_len - 1 {
                ui.add_space(gap);
            }
...
fn render_answer_help(ctx: &egui::Context, metrics: crate::theme::ThemeMetrics, show: &mut bool) {
    egui::Window::new("How to answer")
        ...
        .show(ctx, |ui| {
            ...
            ui.vertical_centered(|ui| {
                if ui.add_sized(egui::vec2(96.0, 36.0), egui::Button::new("Close")).clicked() {
                    *show = false;
                }
            });
        });
...
fn render_filled_slot(
    ...
    !is_locked && response.clicked()
}

fn render_empty_slot(ui: &mut egui::Ui, idx: usize, app: &mut LexivoApp, slot_size: f32) {
